...
class pyramid
{
    ...
    {
        assert (i + 1 < levels ());
        jack_rabbit::reduce2x2 (m_[i], m_[i + 1]);
    }
    ...
    void expand2x2 (size_type i)
    {
        assert (i > 0);
        assert (i < levels ());
        jack_rabbit::expand2x2 (m_[i], m_[i - 1]);
    }
    ...
    void expand2x2_and_blur3x3 (size_type i)
    {
        assert (i > 0);
        assert (i < levels ());
        jack_rabbit::expand2x2_and_blur3x3 (m_[i], m_[i - 1]);
    }
    /// @brief Reduce operation
    /// @param i src level index
    ///
    ...
    {
        assert (i + 1 < levels ());
        jack_rabbit::reduce3x3 (m_[i], m_[i + 1]);
    }
    /// @brief Expand operation
    /// @param i src level index
    ///
    ...
    void expand3x3 (size_type i)
    {
        assert (i > 0);
        assert (i < levels ());
        jack_rabbit::expand3x3 (m_[i], m_[i - 1]);
    }
    ...
    void expand3x3_with_jitter (size_type i, unsigned j)
    {
        assert (i > 0);
        assert (i < levels ());
        jack_rabbit::expand3x3 (m_[i], m_[i - 1], j);
    }
