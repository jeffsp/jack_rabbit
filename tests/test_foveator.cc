...
template<typename T>
const raster<typename T::precision_type> get_blending_map_image (const T &bm, size_t rows, size_t cols)
{
    raster<typename T::precision_type> rm (rows, cols);
...
void test0 ()
{
    ...
    f.foveate ();
    raster<T> d = f.get_foveated_pyramid ()[0];
    //clog << d << endl;
    VERIFY (d (R / 2, 0) == N);
    VERIFY (d (R / 2, C - 1) == N);
    b.set_e2 (C / 4);
...
void test1 ()
{
    ...
    f.set_source_pixels (m.begin (), m.end ());
    const raster<T> &d = f.get_foveated_pyramid ()[0];
    VERIFY (d.rows () == R);
    VERIFY (d.cols () == C);
    VERIFY (d[0] == 0);
    f.foveate ();
